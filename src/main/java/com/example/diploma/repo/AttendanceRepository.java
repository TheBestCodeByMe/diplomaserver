...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    @Query(value = "select a from Attendance a, Schedule s, Calendar c where a.pupilID = ?1 and a.statusId = ?2 and a.lessonID = s.id and s.calendarId = c.id and c.semesterID = ?3")
    List<Attendance> findAllByPupilIDStatusIdAndSem(Long pupilId, Long statusId, int sem);

    @Query(value = "select a from Attendance a, Schedule s, Calendar c where a.classID = ?1 and a.statusId = ?2 and a.lessonID = s.id and s.calendarId = c.id and c.semesterID = ?3")
